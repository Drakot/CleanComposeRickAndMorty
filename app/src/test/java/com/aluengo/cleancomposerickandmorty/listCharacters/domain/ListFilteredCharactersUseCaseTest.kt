...
class ListFilteredCharactersUseCaseTest : BaseTest() {
    private lateinit var mockData: MockData
    private lateinit var repository: Repository
    private lateinit var useCase: ListFilteredCharactersUseCase

    @Before
    fun setUp() {
        repository = mockk()
        useCase = ListFilteredCharactersUseCase(repository)
    ...
    fun `invoke should return repository's listCharacters result success`() = runTest {
        ...
        coEvery { repository.listCharactersWithFilter(request) } returns flowOf(resourceSuccess)

        val result = useCase(request).first()

        TestCase.assertTrue(result is Resource.Success)
        TestCase.assertEquals(charactersDomain, (result as Resource.Success).data)
    }
    ...
    fun `invoke should return repository's listCharacters result error`() = runTest {
        ...
        coEvery { repository.listCharactersWithFilter(request) } returns flowOf(Resource.Error(error))

        val result = useCase(request).first()

        TestCase.assertTrue(result is Resource.Error)
        TestCase.assertEquals(error, (result as Resource.Error).error)
    }
